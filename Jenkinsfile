...
node ('docker-cmd'){
    ...
    dockerlogin()
    dockerrmi("oneforone/docker-mesosphere-base:${env.BRANCH_NAME}.${env.BUILD_NUMBER}")
    dockerbuild("oneforone/docker-mesosphere-base:${env.BRANCH_NAME}.${env.BUILD_NUMBER}")
}
...
node('docker-cmd') {
    dockerlogin()
    dockerpush("oneforone/docker-mesosphere-base:${env.BRANCH_NAME}.${env.BUILD_NUMBER}")
...
switch ( env.BRANCH_NAME ) {
    case "master":
        ...
        node('docker-cmd') {
            ...
            // Erase
            dockerrmi('oneforone/docker-mesosphere-base:latest')

            // Tag
            dockertag("oneforone/docker-mesosphere-base:${env.BRANCH_NAME}.${env.BUILD_NUMBER}","oneforone/docker-mesosphere-base:latest")

            // Push
            slackSend color: 'blue', message: "ORG: ${env.JOB_NAME} #${env.BUILD_NUMBER} - Pushing :latest"
            dockerpush('oneforone/docker-mesosphere-base:latest')
        }
